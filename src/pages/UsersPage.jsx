...
import { Plus, UserCog, Search, X, Save, Mail, Copy, Check, ToggleLeft, ToggleRight, Trash2 } from 'lucide-react'
...
export default function UsersPage() {
  ...
  async function handleCreate() {
    ...
    setSaving(false)
    setShowModal(false)
    setSuccess(`✅ Usuario creado: ${form.email} · Contraseña: ${form.password}`)
  ...
  async function handleDelete() {
    ...
    setDeleting(true)
    // Delete from profiles (auth user deletion requires service role, so we just remove the profile)
    await supabase.from('profiles').delete().eq('id', userToDelete.id)
    setDeleting(false)
    setShowDeleteModal(false)
    setUserToDelete(null)
    setSuccess(`🗑️ Usuario ${userToDelete.email} eliminado del portal.`)
    fetchAll()
  }
  ...
  function copyCredentials() {
    navigator.clipboard.writeText(success.replace('✅ ', ''))
    setCopied(true)
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100">
        {loading ? (
          ...
          <table className="w-full">
            <thead>
              <tr>
                {['Usuario', 'Rol', 'Organización', 'Estatus', 'Acciones'].map(h => (
                  ...
                  <tr key={u.id} className={`hover:bg-slate-50 transition-colors ${!u.is_active ? 'opacity-50' : ''}`}>
                    ...
                    <td className="px-5 py-4">
                      <select value={u.org_id || ''}
                        ...
                        className="text-xs border border-slate-200 rounded-lg px-2 py-1.5 focus:outline-none focus:ring-2 focus:ring-[#2FA4A9] max-w-[160px]">
                        <option value="">Sin organización</option>
                        {orgs.map(o => <option key={o.id} value={o.id}>{o.name}</option>)}
                      </select>
                    </td>
                    <td className="px-5 py-4">
                      <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${u.is_active ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-600'}`}>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">Se muestra en texto para que puedas copiarla y enviársela.</p>
              </div>
              <div>
